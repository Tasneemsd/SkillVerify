...
const BASE_URL = "https://skillverify.onrender.com/api/recruiter";
...
const Recruiter = () => {
  ...
  const fetchRecruiter = async () => {
    try {
      ...
      const res = await axios.get(`${BASE_URL}/profile?email=${user.email}`, {
      ...
      console.error(err);
      setError("Failed to load recruiter info");
    }
  };
  ...
  const fetchJobs = async () => {
    try {
      const res = await axios.get(`${BASE_URL}/jobs/mine`, { headers: getAuthHeaders() });
      setJobs(res.data);
    } catch (err) {
      console.error(err);
  ...
  const createJob = async (e) => {
    ...
    try {
      const jobData = {
        ...newJob,
        skills: newJob.skillsRequired.split(",").map((s) => s.trim()),
      };
      await axios.post(`${BASE_URL}/jobs`, jobData, { headers: getAuthHeaders() });
  ...
  const updateSettings = async (e) => {
    e.preventDefault();
    try {
      await axios.post(`${BASE_URL}/profile`, settings, { headers: getAuthHeaders() });
