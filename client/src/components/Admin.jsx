...
const Admin = () => {
  ...
  // --- Utilities for robust responses ---
  const extractArray = (data, possibleKeys = []) => {
    // Returns the first array found in common envelope fields
    if (!data) return [];
    ...
    // sometimes API returns { data: { data: [...] } } or nested
  ...
  const fetchJobs = async () => {
    try {
      ...
      const arr = extractArray(res.data, ["jobs"]);
      setJobs(arr);
    } catch (err) {
  ...
  const fetchMockInterviews = async () => {
    try {
      ...
      const arr = extractArray(res.data, ["mockInterviews", "interviews"]);
  ...
  const fetchCandidates = async () => {
    // kept for backward compatibility - some logic uses 'candidates' list separately
    try {
      const res = await axios.get(`${BASE_URL}/admin/students`, { headers: getAuthHeaders() });
      console.log("fetchCandidates response:", res.data);
      const arr = extractArray(res.data, ["students", "users"]);
      setCandidates(normalizeUsers(arr));
      // also ensure users state is consistent
      setUsers(normalizeUsers(arr));
    } catch (err) {
  ...
  const confirmVerification = async () => {
    try {
      if (!selectedStudent?._id && !selectedStudent?.id) throw new Error("No student selected");
      const sid = selectedStudent._id || selectedStudent.id;
      await axios.post(`${BASE_URL}/admin/verify-student`, { studentId: sid }, { headers: getAuthHeaders() });
      // update local state safely (both _id and id fallback)
      setUsers((prev) =>
        prev.map((u) => (u._id === sid || u.id === sid ? { ...u, verified: true, isVerified: true } : u))
      );
      setCandidates((prev) =>
        prev.map((u) => (u._id === sid || u.id === sid ? { ...u, verified: true } : u))
      );
      setShowModal(false);
  ...
  const toggleRecruiterApproval = async (recruiterId) => {
    try {
      const recruiter = users.find((u) => u._id === recruiterId || u.id === recruiterId);
      ...
      setUsers((prev) => prev.map((u) => (u._id === recruiterId || u.id === recruiterId ? { ...u, status: newStatus } : u)));
    } catch (err) {
  ...
  const updateInterviewStatus = async (interviewId, status) => {
    try {
      ...
      setMockInterviews((prev) => prev.map((i) => (i._id === interviewId || i.id === interviewId ? { ...i, status } : i)));
    } catch (err) {
  ...
  const updateApplicationStatus = async (appId, status) => {
    try {
      ...
      setApplications((prev) => prev.map((a) => (a._id === appId || a.id === appId ? { ...a, status } : a)));
    } catch (err) {
  ...
  // --- Load All Data on mount (and optionally refresh per tab)
  useEffect(() => {
    const token = getAuthToken();
    if (!token) {
      navigate("/login");
  ...
  // Optional: re-fetch when a specific tab becomes active to ensure fresh data
  useEffect(() => {
    if (activeTab === "students") fetchUsers();
    if (activeTab === "candidates") fetchCandidates();
    if (activeTab === "recruiters") fetchUsers(); // recruiters are in users
    if (activeTab === "interviews") fetchMockInterviews();
    if (activeTab === "applications") fetchApplications();
    if (activeTab === "dashboard") {
      // nothing extra, dashboard uses derived data
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [activeTab]);
  ...
  // --- Render (UI preserved; only logic improved) ---
  return (
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        {isLoading ? (
          ...
          <>
            ...
            {/* Students (original 'Manage Students' UI preserved) */}
            ...
            {activeTab === "nonVerified" && (
              <div className="overflow-auto">
                ...
                {students.filter(s => !s.verified).length === 0 ? (
                  <p className="text-gray-500 p-4">No non-verified students.</p>
                ) : (
                  <table className="min-w-full bg-white rounded shadow">
                    ...
                    <tbody>
                      {students.filter(s => !s.verified).map((stu) => (
                        <tr key={stu._id || stu.email} className="border-t hover:bg-gray-50">
                    ...
                    <tbody className="divide-y divide-gray-200">
                      {recruiters.map((recruiter) => (
                        <tr key={recruiter._id || recruiter.email} className="hover:bg-gray-50 transition-colors duration-150">
                          ...
                          <td className="px-6 py-4 whitespace-nowrap">
                            <button
                              onClick={() => toggleRecruiterApproval(recruiter._id || recruiter.id)}
                        ...
                        <tbody className="divide-y divide-gray-200">
                          {mockInterviews.map((interview) => (
                            <tr key={interview._id || `${interview.studentId}-${interview.date}`} className="hover:bg-gray-50 transition-colors duration-150">
                              ...
                              <td className="px-6 py-4 whitespace-nowrap">
                                <button
                                  onClick={() => updateInterviewStatus(interview._id || interview.id, "Completed")}
                                ...
                                <button
                                  onClick={() => updateInterviewStatus(interview._id || interview.id, "Cancelled")}
                        ...
                        <tbody className="divide-y divide-gray-200">
                          {applications.map((app) => (
                            <tr key={app._id || `${app.studentId}-${app.jobId}`} className="hover:bg-gray-50 transition-colors duration-150">
                              ...
                              <td className="px-6 py-4 whitespace-nowrap">
                                <button
                                  onClick={() => updateApplicationStatus(app._id || app.id, "Selected")}
                                  className="inline-flex items-center px-3 py-1 bg-green-500 text-white rounded-lg text-sm hover:bg-green-600 transition-colors mr-2"
                                >
                                  Select
                                </button>
